...
- **⚠️ CONTENT IS STILL THE STUDENTS CLONE** (eyebrow "For Students", hero "Become a builder.", the 11 *student* program cards) — placeholder pending the real Faculty/Researchers copy. Swap it the same way as About (§24): eyebrow `set_text`, headline words (create/apply `if-hero-word*` markers), `if-hero-lead`, and edit the cards. The user will provide Faculty content later.
