...
- **Stage 01 photo — aspect + accent bars (2026‑07‑06, user).** User: match the design's photo styling + "keep a better aspect ratio, maybe widescreen TV ratio." Set the photo to **16:9** on `if-stg-photo` (`aspect-ratio:16 / 9`, `width:100%`; **removed** the fixed `height` clamp). The design frames the photo with a **red‑left + gold‑bottom‑left L‑bracket**: red = `if-stg-photo` `border-left:6px #e21833` (thickened from 4px); gold = new **`if-stg-goldbar`** (absolute, `left:0 bottom:0`, `width:44%`, `height:12px`, `#ffd200`) inside the now `position:relative` frame. Verified published via harness measure: photo **508×286 (16:9)**, red **6px**, gold **221×12**. ⚠️ **`if-stg-goldbar` is an empty div → it INFLATES huge in the Designer canvas / `element_snapshot` (same empty‑div quirk as the §28 glyph) but renders correct (12px) PUBLISHED** — verify sizes by measuring rects in the harness, not by the snapshot. New class: `if-stg-goldbar`.
