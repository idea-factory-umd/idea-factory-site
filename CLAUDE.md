...
User asks (verbatim intent): "get rid of the yellow hatch"; "remove the red color from the text when it's not being hovered over — you can leave it red on hover … make red the hover color in this hamburger/responsive menu, along with the 'grow', overall." Implemented in the `@media (max-width:991px)` block of `idea-factory.css`:
- **HOVER (any item — top link, toggle, or sub‑item):** `color:#e21833` (brand red) + `transform:scale(1.07)` (grow), `transform-origin:left center`; **no gold hatch, no background, no box‑shadow.**
- **CURRENT page:** red left bar via **`::before`** (`.if-navmenu.if-navmenu a.w--current::before` = 4px `#e21833`), with **non‑red resting text matched to siblings** — top‑level `#000`, sub‑item `#7f7f7f`. Red is hover‑only; hovering the current item reddens it too (the `:hover` rule is placed AFTER the current‑color rule at equal specificity `(0,4,0)` so it wins the tie — **do not re‑add `.if-dd-list a.w--current` to the mobile color selector, that's `(0,4,1)` and would beat hover, freezing the current item gray on hover**).
- **st0 interaction:** doubled `.if-navmenu.if-navmenu …:hover` `(0,4,0)` out‑specifies the in‑site st0 `!important` hover rule `(0,3,0)` (which sets a red bar + `transform:none`), so grow applies and no bar shows on hover. Marker is `::before` (not box‑shadow) so it survives the hover `box-shadow:none` and can't fringe when the row scales.
- **Verified offline** (mobile 480px, menu force‑opened, CDP forced‑hover, transitions settled): non‑current resting = gray no‑bar; non‑current hover = red + `scale(1.07)` no hatch; current resting = gray + red `::before` bar; current hover = red + grow + bar; 0 JS `pageerror`. Screenshots `MOB_contrast.png` / `MOB_currenthover.png` in scratchpad.
