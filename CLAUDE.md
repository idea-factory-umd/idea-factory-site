...
- **CURRENT PIN = `c1baf6f` (v1.2.1).** Served at `…/idea-factory-site@c1baf6f/idea-factory.{css,js}`.
- **AWAITING USER:** bump the 2 refs in Project Settings to `@c1baf6f` + publish, then confirm the search modal scrolls. (Version history: c0fd5ed=v1.1.0 broken → c7dc070=v1.2.0 restored hovers/count-up → c1baf6f=v1.2.1 CSE scroll fix.)
