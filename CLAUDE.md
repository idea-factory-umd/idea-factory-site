...
- **HOVER (any item — top link, toggle, or sub‑item):** `transform:scale(1.07)` (grow), `transform-origin:left center`; **no gold hatch, no background, no box‑shadow, and NO color change** — the text keeps its resting color. Per‑type color rules (`.if-nav-link/.w-dropdown-toggle:hover{color:#000}`, `.if-nav-sublink:hover{color:#7f7f7f}`) are there ONLY to override st0's `!important` hover‑darkening so nothing shifts but size.
