...
- **Top lead "Every great idea…" = new global class `if-premise-lead`** (id `76d09305‑2167‑f183‑6ad8‑7eba59f0bfc7`), applied to the top h2 (`b1bb0b0c…d858`; its child Span `…d84f` inherits). User wanted "something different FOR NOW… maybe Georgia… artistic, somewhat LIKE a callout quote, a little smaller; ignore the mockup formatting." Built as a **serif italic pull‑quote**: `font-family:Georgia,"Times New Roman",serif; font-style:italic; font-weight:400; font-size:clamp(26px,3.6vw,46px)` (a bit smaller than manifesto's `clamp(30,4.6vw,64)`); `line-height:1.25; color:#1a1a1a; max-width:32ch;` **red left accent bar** `border-left:3px solid #e21833; padding-left:clamp(18px,1.8vw,28px)`.
  - **⚠️ CRITICAL — the shared CSS would otherwise force it to Interstate.** §22's broad rule `[class^="if-"]{font-family:var(--if-sans)!important}` overrides ANY `if-` class's Designer font. So `if-premise-lead` was **added to the serif exception line** in `idea-factory.css` (line 72): `…,.if-mani-p,.if-premise-lead,.if-prog-desc,.if-tag-p{font-family:var(--if-serif)!important}` (comment count 6→7). Without this the lead renders Interstate‑italic (fake), not Georgia. This is the documented "append a serif class" path (§22). **→ shared‑file change: pushed to `main`/dev + GitHub Pages.**
