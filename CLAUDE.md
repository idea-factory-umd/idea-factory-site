...
- **HERO CONTENT — done 2026‑07‑02 (native, published).** Swapped the black‑left‑panel content to the Faculty design (hero‑right building photo unchanged): eyebrow **"For Faculty & Researchers"** (`set_text` on the eyebrow's text String, bar preserved), headline **"Take your / research / to market."** (3 line‑units in the 3 existing word spans + one new `if-h1-brk` div; "to market." is **gold**), lead **"Funding, customer‑discovery training, and venture support to move a discovery out of the lab and into the world. Five ways forward."** ("Five ways forward." = the existing `if-hero-lead-strong` gold span, text swapped).
  - **⭐ GOLD read‑word technique (new, reused going forward):** the hero‑reader JS only lights **red** (`if-hero-word-red`→`if-lit-red`, §14). To get a **gold** highlight WITHOUT touching the minified shared JS, the gold word carries **both** `if-hero-word-red` (drives the JS reveal + opacity‑hold, unchanged) **and** a new marker `if-hero-word-gold`; a shared‑CSS rule `.if-hero-word-red.if-hero-word-gold.if-lit-red{color:#ffd200}` (specificity 0,3,0, beats the 0,2,0 red rule) recolors the lit state red→gold. White at rest → fades to gold on read → holds. `if-hero-word-gold` was `create_style`'d as an empty marker (id `fae9f5e3…`). This is the pattern for any future gold‑highlight headline.
  - Headline spans (Faculty page, component=page id): "Take your" `01cbe865…5b34` (`if-hero-word`), break `de36bf41…ab96`, "research" `9c835a7b…a100` (`if-hero-word if-hero-word-delay`), new break `f449566b…95d6`, "to market." `46c40c3b…7949` (`if-hero-word if-hero-word-red if-hero-word-gold`).
...
- **⚠️ The DIRECTORY CARDS below the hero are STILL the Students clone** (11 *student* program cards). Only the hero‑left changed (per the user's ask). Swap the cards when the Faculty/Researchers program list is provided.
