...
- **⭐ Hero height — FIXED 2026‑07‑02.** It had been inheriting Home's `if-hero-grid` `min-height:min(86vh,760px)` — too tall for Students (only 3 items in `if-hero-left`: eyebrow, H1, lead; `justify-content:space-between` spread them with big empty gaps). The **original design `project/ui_kits/website/StudentsPage.jsx` specs the Students hero grid at `min-height:min(64vh,540px)`** (we'd simply copied Home's value when duplicating). Fixed with a **Students‑only combo `.if-hero-grid.if-hero-compact` = `min-height:min(64vh,540px)`** (native Designer style via MCP; applied to the Students hero‑grid element `9636f821‑…‑b0` only). Home's base `.if-hero-grid` stays `min(86vh,760px)` (verified: Home grid carries only `if-hero-grid`, Students carries both). Published + verified live (compiled `.if-hero-grid.if-hero-compact{min-height:min(64vh,540px)}`).
  - **Other StudentsPage.jsx deltas still mimicking Home (NOT yet applied — align via the same compact combo if fuller design fidelity is wanted):** headline `clamp(52px,7.5vw,104px)` in the design vs the copied `if-hero-h1` `clamp(58px,8.5vw,132px)` (design's Students H1 is smaller, pairs with the shorter hero); grid columns `1.05fr 0.95fr` vs `1.15fr 0.85fr`; left padding `clamp(28px,5vw,64px)` vs `…72px`; hero‑right `min-height:280px` vs `360px` (moot — row is taller). Design also says "9 ways in." but the built copy uses "11 ways in." (content choice, keep).
