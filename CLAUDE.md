...
- **Because the Faculty "to market." span had been published CLEAN (`if-hero-word-gold` only), the stale JS couldn't light it → gold was momentarily WHITE on hover.** Fix: **temporarily re‑added `if-hero-word-red`** to that span (`46c40c3b…7949` → `if-hero-word if-hero-word-red if-hero-word-gold`) so the *currently‑served* files light it gold (old JS adds `if-lit-red`; the still‑served hacky CSS rule `.if-hero-word-red.if-hero-word-gold.if-lit-red{#ffd200}` paints gold). Gold works NOW. **TODO once Pages recovers:** strip `if-hero-word-red` again for the clean end‑state (the clean JS/CSS on `main` already support `if-hero-word-gold` alone; the new `.if-hero-word-gold.if-lit-gold` rule wins even with red present, so gold holds through the transition).
