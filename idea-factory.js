...
/* ===== ifResizeEngine: single coordinated `resize` dispatcher (added 2026-09-24, jank fix) =====
 * Same problem the scroll engine above already solved, but for resize: 16 separate modules below
 * each registered their OWN independent `window.addEventListener('resize', ...)`, several with NO
 * debounce - running their full measure+write logic on every raw event. On mobile Safari/Chrome,
 * the address bar collapsing/expanding DURING a scroll gesture fires real `resize` events - often
 * 10-20 in a row over one toolbar animation - so a single mobile scroll could trigger up to 16
 * uncoordinated handlers, repeatedly, competing with the compositor thread mid-scroll. Measured as
 * the dominant cause of site-wide scroll jank (2026-09-24).
 * Fix: ONE resize listener, rAF-batched, that runs every registered callback at most once per real
 * animation frame - same functions, same end result, just coalesced instead of fired per-event. */
try {
window.ifResizeEngine = (function(){
  var fns = [], ticking = false;
  function run(){
    ticking = false;
    for (var i=0; i<fns.length; i++){
      try { fns[i](); }
      catch (e) { try { console && console.warn && console.warn('[idea-factory] resize-engine callback error:', e); } catch(_){} }
    }
  }
  window.addEventListener('resize', function(){ if(!ticking){ ticking = true; requestAnimationFrame(run); } }, {passive:true});
  return { add: function(fn){ fns.push(fn); } };
})();
} catch (_e) { try { console && console.warn && console.warn('[idea-factory] resize-engine init error:', _e); } catch (_) {} }
...
(function () {
  ...
  function initHeaderGoldBar(){
    ...
    } else { ifResizeEngine.add(measure); }
  }
...
(function(){
  ...
  function init(){ adjust(); ifResizeEngine.add(onR); if(document.fonts && document.fonts.ready && document.fonts.ready.then){ document.fonts.ready.then(adjust); } }
  if(document.readyState!=='loading')init();else document.addEventListener('DOMContentLoaded',init);
})();
...
(function(){
  ...
  function init(){ centerV(); ifResizeEngine.add(onR); if(document.fonts && document.fonts.ready && document.fonts.ready.then){ document.fonts.ready.then(centerV); } }
  if(document.readyState!=='loading')init();else document.addEventListener('DOMContentLoaded',init);
})();
...
(function(){
  ...
  function init(){ fitHeaderLeft(); ifResizeEngine.add(onR); if(document.fonts && document.fonts.ready && document.fonts.ready.then){ document.fonts.ready.then(fitHeaderLeft); } }
  if(document.readyState!=='loading')init();else document.addEventListener('DOMContentLoaded',init);
})();
...
try {
  (function(){
    ...
    if(navbarIn && window.ResizeObserver){
      ...
      ifResizeEngine.add(scheduleFit);
    }
  })();
