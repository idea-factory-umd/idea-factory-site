  ...
  function init(){
    ...
    wraps.forEach(function(wrap){
      ...
      ifScrollEngine.add({
        read:function(){
          var headerH=header?header.getBoundingClientRect().height:0;
          /* +24 (0 on CBSCF pages) must match the "cross-page-anchor-scroll" module's own
             goTo() landing offset EXACTLY - these links are #goto:<id> anchors, so that's the
             offset a click actually lands them at; +12 on top is the same "just past the
             ...
             on What You Get). CBSCF's own gap was zeroed out 2026-09-17 - kept in sync here. */
          var gap=/^\/cbscf-/.test(location.pathname)?0:24;
...
(function(){
  ...
  function goTo(id){
    ...
    /* CBSCF-only: land flush against the header (no gap) - fixes a 24px reveal of the
       preceding section reported on CBSCF's #goto: links (2026-09-17). Every other site's
       links through this same shared function keep the original 24px gap unchanged. */
    var gap = /^\/cbscf-/.test(location.pathname) ? 0 : 24;
