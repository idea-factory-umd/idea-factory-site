...
(function(){
  ...
  if(sec && !sec.__ifsm){
    ...
    if(!reduce){
      ...
      /* PERF (2026-09-09): willChange used to be reassigned the SAME value every single
         write-frame while animating (a per-frame audit found this). Now gated on an actual
         state transition (txtWC/imgsWC) - set once on entering the animating window, cleared
         once on leaving it. No change to the animation itself. */
      var txtWC=false,imgsWC=false;
      ifScrollEngine.add({
        read: function(){
        ...
        write: function(m){
          ...
          if(txt && !txtLocked){
            ...
            if(p>0 && p<1){ if(!txtWC){txt.style.willChange='transform,opacity';txtWC=true;} }
            txt.style.transform='translateY('+shift.toFixed(1)+'px) scale('+sc.toFixed(4)+')';
            txt.style.opacity=(p*p).toFixed(3);
            if(p>=1){txtLocked=true;txt.style.transform='translateY(0px) scale(1.06)';txt.style.opacity='1';txt.style.willChange='auto';txtWC=false;}
          }
          if(imgs && imgs.length){
            ...
            if(animating && !imgsWC){ for(var i=0;i<imgs.length;i++) imgs[i].style.willChange='transform'; imgsWC=true; }
            else if(!animating && imgsWC){ for(var i2=0;i2<imgs.length;i2++) imgs[i2].style.willChange='auto'; imgsWC=false; }
            for(var i3=0;i3<imgs.length;i3++){ imgs[i3].style.transform=tf; }
          }
        }
      });
      ifScrollEngine.kick();
...
(function(){
  ...
  function init(){
    ...
    /* PERF (2026-09-09): same willChange-every-frame issue as stage-parallax above, found by
       the same audit - gated on a state transition instead of reassigning the same value every
       write-frame. No change to the animation itself. */
    var txtWC=false,imgWC=false;
    ifScrollEngine.add({
      ...
      write: function(m){
        ...
        if(txt && !txtLocked){
          ...
          if(p>=1){txtLocked=true;txt.style.transform='translateY(0px) scale(1.05)';txt.style.opacity='1';txt.style.willChange='auto';txtWC=false;}
        }
        if(img){
          ...
          var eased=1-Math.pow(1-peak,3), cur=MIN+(MAX-MIN)*eased;
          var animating = peak>0 && peak<1;
          if(animating && !imgWC){ img.style.willChange='transform'; imgWC=true; }
          else if(!animating && imgWC){ img.style.willChange='auto'; imgWC=false; }
...
(function(){
  function init(){var bars=document.querySelectorAll('.if-stg-goldbar, .if-syn-goldbar');if(!bars.length)return;
    ...
    /* PERF (2026-09-09): write() used to call getBoundingClientRect() per bar itself - a
       read-inside-write violation of the engine's own contract (found via CLAUDE.md follow-up
       audit). All per-bar geometry is now read once, up front, in read(); write() only
       computes from that and sets transform. classList.contains() stays in write() - it's a
       class check, not a layout-forcing read. Math/values unchanged. */
    ifScrollEngine.add({
      read: function(){
        var vh=window.innerHeight||document.documentElement.clientHeight,rects=[],j;
        for(j=0;j<bars.length;j++){var r=bars[j].getBoundingClientRect();rects.push({top:r.top,h:r.height});}
        return {vh:vh, rects:rects};
      },
      write: function(m){
        if(!m) return;
        var j;
        for(j=0;j<bars.length;j++){var _b=bars[j],r=m.rects[j];var p=(m.vh-r.top)/(m.vh+r.h);if(p<0)p=0;if(p>1)p=1;var mx=_b.classList.contains('if-syn-goldbar')?60:MAX;var w=MIN+(mx-MIN)*p;_b.style.transform='scaleX('+(w/mx)+')';}
      }
    });
    ifScrollEngine.kick();
  }
